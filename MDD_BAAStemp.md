...
### IMPORTANT: this field needs review and needs to be implemented in context with the rest of the document

# Architecture and Strategy: mini-baas (App Factory)
...
## 4. The Universal Abstraction Engine (Database Agnosticism)

For the Metadata-Driven model to be truly powerful, the system must hide the complexity of the underlying database from the Tenant. The goal is a true agnostic adapter: the user sends a generic data request, and the engine decides how to execute it in SQL or NoSQL.



### The Technical Challenge
Abstracting databases implies a clash of paradigms. Simulating relational operations (JOINs) in NoSQL, or simulating the flexibility of nested documents in SQL, carries massive performance penalties if not designed correctly.

### Strategic Recommendation (Implementation Phases)

**Phase 1: Strategy Pattern based on AST (Abstract Syntax Tree)**
The tenant will communicate with the BaaS using a unified query language (REST JSON or GraphQL). The BaaS will parse this request into a generic AST and send it to the specific Driver configured in the Tenant's metadata.

/**
 * Phase 1: The tenant must explicitly choose their database engine 
 * in their Master Document metadata.
 */
...
**Phase 2 (Future): Auto-Provisioning and Intelligent Routing**
We will delegate the database engine decision to the BaaS itself. By analyzing the schema uploaded by the Tenant, the system will decide to provision a PostgreSQL database (if it detects high relationality) or MongoDB (if it detects deeply nested unstructured data structures), without human intervention.

---

## 5. Specific System Documentation

The technical architecture is divided into three strictly separated planes:

### 5.1. Control Plane (Governance)
* **Engine:** MongoDB (`baas-system-db`).
* **Responsibility:** Store the Master Documents (`TenantMetadata`). It defines the "who", the "how", and the permissions of each Tenant.

### 5.2. Data Plane (Tenant Storage)
* **Engine:** PostgreSQL (`baas-tenant-db`), configurable to other engines via adapters.
* **Responsibility:** Physical storage of the end-users' data for the Tenant.

### 5.3. Runtime and Cache Plane (Execution)
* **Engine:** NestJS + `isolated-vm` (API) and Redis (`baas-redis`).
* **Responsibility:** NestJS intercepts requests, resolves the Tenant context using Redis caching, and compiles AST queries for the Data Plane. User code Hooks are executed securely in isolated V8 environments to avoid compromising the main Node.js thread.
